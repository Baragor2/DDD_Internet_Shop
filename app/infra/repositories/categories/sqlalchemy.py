...
from infra.repositories.categories.converters import (
    convert_category_model_to_entity,
...
class SqlAlchemyCategoriesRepository(BaseCategoriesRepository):
    ...
    async def add_category(self, category: Category) -> None:
        query = insert(Categories).values(convert_category_entity_to_document(category))
    ...
    async def update_category_title(
        ...
        select_query = select(Categories).where(
            Categories.title == new_title.as_generic_type()
        )
