...
from logic.exceptions.categories import (
    CategoryWithThatTitleAlreadyExistsException,
    CategoryWithThatTitleNotExistsException,
)
...
@dataclass(frozen=True)
class ChangeCategoryTitleCommandHandler(
    CommandHandler[ChangeCategoryTitleCommand, Category]
):
    uow_factory: Callable[[], UnitOfWork]
    ...
    async def handle(self, command: ChangeCategoryTitleCommand) -> Category:
        async with self.uow_factory() as uow:
            ...
            if not await categories_repository.check_category_exists_by_title(
                old_title
            ):
            ...
            changed_category = await categories_repository.update_category_title(
                old_title, new_title
            )
